...
def run_and_collect_results(
    ...
    Args:
        ...
        user_target_events_details (dict): ユーザーが申し込む公演の詳細
        duplicate_config (dict): 重複当選の設定
        case_name (str): シミュレーションケースの名前（ログ表示用）
        stages_def (list): 各選考ステージの定義のリスト
            各要素は (name, applicant_core_fan_ratio, additional_applicants, weight) の形式
